// ============================================
// BOOKING PRICING ENGINE - CLEAN LIVE NYC
// ============================================
...
// ============================================
// Source: Копия_CleanLine_flat_rates_v1_xlsx_-_FlatRates.tsv
...
export const ADDONS = {
  // Cleaning Upgrades
  fridge: { id: 'fridge', name: 'Refrigerator Inside', price: 40, time: 30, maxQty: 1 },
  oven: { id: 'oven', name: 'Oven Inside', price: 40, time: 30, maxQty: 1 },
  cabinets: { id: 'cabinets', name: 'Interior Cabinets', pricePerUnit: 20, time: 20, maxQty: 4, hasQuantity: true },

  // Windows & Surfaces
  windows: { id: 'windows', name: 'Inside Windows', pricePerUnit: 15, time: 15, maxQty: 10, hasQuantity: true },
  blinds: { id: 'blinds', name: 'Blinds Dusting', pricePerUnit: 25, time: 25, maxQty: 8, hasQuantity: true },
  baseboards: { id: 'baseboards', name: 'Baseboards Detail', pricePerBedroom: 20, time: 20, autoCalculated: true },
  walls: { id: 'walls', name: 'Wall Spot Cleaning', pricePerUnit: 30, time: 30, maxQty: 6, hasQuantity: true },

  // Laundry & Dishes
  laundry: { id: 'laundry', name: 'Laundry Service', pricePerUnit: 35, time: 45, maxQty: 3, hasQuantity: true },
  dishes: { id: 'dishes', name: 'Dish Washing', price: 25, time: 30, maxQty: 1 },
  bedLinen: { id: 'bed-linen', name: 'Change Bed Linens', pricePerUnit: 25, time: 20, maxQty: 4, hasQuantity: true },

  // Special Items
  balcony: { id: 'balcony', name: 'Balcony/Patio', pricePerUnit: 40, time: 35, maxQty: 2, hasQuantity: true },
  garage: { id: 'garage', name: 'Garage Sweep', price: 50, time: 40, maxQty: 1 },

  // Deep Extras (only for Deep & Move services)
  mattress: { id: 'mattress', name: 'Mattress Vacuuming', pricePerUnit: 30, time: 25, maxQty: 3, hasQuantity: true, showOnlyFor: ['deep', 'move'] },
  upholstery: { id: 'upholstery', name: 'Upholstery Spot Clean', pricePerUnit: 45, time: 35, maxQty: 2, hasQuantity: true, showOnlyFor: ['deep', 'move'] },
};
...
export function getFlatRatePrice(
  ...
  const pricing = FLAT_RATES.find(
    (r) => r.sizeId === sizeId && r.service === serviceId
  );
...
export function calculateAddOnsTotal(
  ...
  selectedAddons.forEach((addon) => {
    const { price, time } = calculateAddonPrice(
      addon.id,
      addon.quantity || 1,
      bedrooms
    );
...
export function calculateBookingPrice(data: BookingData): PricingResult {
  ...
  if (service.type === 'flat') {
    ...
    const flatRate = getFlatRatePrice(
      data.serviceId as 'standard' | 'deep' | 'move',
      data.bedrooms,
      data.bathrooms
    );
  ...
  if (service.type === 'hourly') {
    ...
    const totalPrice = calculateHourlyPrice(
      data.serviceId as 'custom' | 'organizing',
      data.hours,
      data.teamSize || 1
    );
